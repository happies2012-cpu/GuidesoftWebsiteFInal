...
import { Sparkles, Users, Code, Server, Smartphone, Cloud, GitBranch, CheckCircle, Award, Calendar, ArrowRight, Lightbulb, Shield, TrendingUp, Target, Layout, Layers } from "lucide-react"; // Added Layers
import { mainNavigation, NavItem } from "@/lib/navigation-data";

const HireUsOverview = () => {
  ...
  const getIconForTitle = (title: string) => {
    ...
    if (lowerTitle.includes("frontend")) return Code;
    if (lowerTitle.includes("backend")) return Server;
    ...
    if (lowerTitle.includes("devops")) return GitBranch;
    if (lowerTitle.includes("qa")) return CheckCircle;
    if (lowerTitle.includes("remote")) return Cloud;
    return Users; // Default icon
  };

  // Extract top-level hire us categories
  const hireUsCategory = mainNavigation.find(nav => nav.title === "Hire Us");
  const hireUsCategoriesToDisplay = hireUsCategory?.children || [];

  const hireUsOptionsToDisplay = hireUsCategoriesToDisplay.map((category, index) => ({
    icon: getIconForTitle(category.title),
    title: category.title,
    description: `Find ${category.title.toLowerCase()} experts to augment your team and accelerate your projects.`,
    href: category.href || "#",
  }));
      ...
      >
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 backdrop-blur-sm border border-primary/20 mb-8 animate-fade-in">
            ...
            <span className="text-sm font-medium text-primary">Dedicated & Expert Developers</span>
          </div>
          ...
          <p className="text-xl text-foreground max-w-3xl mx-auto animate-fade-in">
            Access top-tier talent to scale your team and accelerate your project delivery with flexible engagement models.
          </p>
        </div>
      </motion.section>

      {/* Hire Us Options Grid */}
      <motion.section
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.3 }}
      ...
      >
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {hireUsOptionsToDisplay.map((option, index) => (
              <motion.div
                key={index}
              ...
              >
                ...
                >
                  ...
                  <CardContent className="p-8 relative z-10 h-full flex flex-col">
                    ...
                    <h3 className="text-xl font-semibold mb-4 text-foreground group-hover:text-primary transition-colors">{option.title}</h3>
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <div className="max-w-3xl mx-auto p-12 rounded-3xl bg-background/10 backdrop-blur-xl border border-primary/20 shadow-[0_0_60px_-15px_hsl(var(--gradient-primary-start)/0.5)]">
            ...
            <p className="text-xl text-muted-foreground mb-8">
              Connect with our pool of pre-vetted, highly skilled developers today.
            </p>
