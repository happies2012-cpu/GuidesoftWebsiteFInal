...
import { Sparkles, Plane, Hotel, Car, Ship, Globe, Code, CheckCircle, Users, Award, Calendar, ArrowRight, Lightbulb, TrendingUp, Target, Layout } from "lucide-react";
...
const TravelOverview = () => {
  ...
  const getIconForTitle = (title: string) => {
    ...
    if (lowerTitle.includes("booking engine")) return Plane;
    if (lowerTitle.includes("portal development")) return Globe;
    if (lowerTitle.includes("api integration")) return Code;
    if (lowerTitle.includes("hotel booking")) return Hotel;
    if (lowerTitle.includes("car rental")) return Car;
    if (lowerTitle.includes("cruise")) return Ship;
    if (lowerTitle.includes("custom travel")) return Lightbulb;
    return TrendingUp; // Default icon
  };

  // Extract top-level travel categories
  const travelCategory = mainNavigation.find(nav => nav.title === "Travel");
  const travelCategoriesToDisplay = travelCategory?.children || [];

  const travelSolutionsToDisplay = travelCategoriesToDisplay.map((category, index) => ({
    icon: getIconForTitle(category.title),
    title: category.title,
    description: `Explore our ${category.title.toLowerCase()} solutions to elevate your travel business.`,
    href: category.href || "#",
  }));
      ...
      >
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 backdrop-blur-sm border border-primary/20 mb-8 animate-fade-in">
            ...
            <span className="text-sm font-medium text-primary">Innovating Travel Technology</span>
          </div>
          ...
          <p className="text-xl text-foreground max-w-3xl mx-auto animate-fade-in">
            Empowering the travel industry with robust booking engines, seamless integrations, and digital transformation.
          </p>
        </div>
      </motion.section>
      ...
      >
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {travelSolutionsToDisplay.map((solution, index) => (
              <motion.div
                key={index}
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <div className="max-w-3xl mx-auto p-12 rounded-3xl bg-background/10 backdrop-blur-xl border border-primary/20 shadow-[0_0_60px_-15px_hsl(var(--gradient-primary-start)/0.5)]">
            ...
            <p className="text-xl text-muted-foreground mb-8">
              Partner with us to build cutting-edge travel technology solutions that drive growth and enhance customer experiences.
            </p>
